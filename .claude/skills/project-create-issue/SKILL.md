...
as a field - more groups and cross-cutting groups (touching multiple services, say) trend toward
a larger size than a single-file, single-group change.
...
- **Labels**: match discovered label names/descriptions against the change's content by keyword
  overlap (e.g. a label named `api` or `frontend` for a change touching `catalog-api`). Apply
