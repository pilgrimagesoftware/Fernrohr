...
- [ ] 9.1 Run the full app on Linux (Wayland and X11 if available): connect to a real cluster, open two Pods panels in two windows, confirm one shared watch, stream a pod's logs, quit and relaunch to confirm layout restore - shared-watch sub-check re-verified on macOS against App commit `bee6130` (2026-09-24): opened a second window with `cmd-n`, its Pods panel showed the same live data as the first immediately, no second connect/relist. Quit/relaunch layout restore: found and fixed a real bug (App commit `785b346`) - `save()` read window geometry from `cx.windows()` inside `on_app_quit`, but under `QuitMode::LastWindowClosed` that hook fires after every window is already closed, so it always wrote `windows = []`; now captured via each window's `on_window_should_close` hook and merged in `save()`. Re-verified manually: two windows, moved/resized, both closed, quit, relaunch - both came back in their last positions/sizes. No Linux box available to test Wayland/X11, and log streaming still can't be checked - `logs.rs` is fully implemented and unit-tested but `shell.rs` still hardcodes `PlaceholderPanel` for the second dock pane, so there is no Logs panel reachable from the UI yet; needs a follow-up task to wire it in before that sub-check is possible
