...
- [ ] 9.1 Run the full app on Linux (Wayland and X11 if available): connect to a real cluster, open two Pods panels in two windows, confirm one shared watch, stream a pod's logs, quit and relaunch to confirm layout restore - all four sub-checks now verified on macOS: shared watch (App `bee6130`), quit/relaunch layout restore after fixing a real bug where `save()` read `cx.windows()` inside `on_app_quit`, which always ran after every window had already closed under `QuitMode::LastWindowClosed` (App `785b346`), and log streaming after wiring a real `LogsPanel` into the dock in place of `PlaceholderPanel` - clicking a Pods row now streams that pod's logs live (App `ab84275`). Only Linux/Wayland/X11 remains, and there's no Linux box available in this environment to run it
...
- [x] 9.3 Confirm no trait/abstraction in the diff has only one implementation and no unused "for later" seam remains; note anything deliberately kept with a one-line reason - findings (App `bee6130`, `785b346`, `ab84275`): removed `runtime::drain_coalescing` (built, tested, never called in production - `pods.rs`'s doc comment wrongly described the real `drain` as "the coalescing drain") and `pods::apply_and_notify` (dead duplicate of logic already inlined in `watch_all_namespaces`); found and fixed three fully-built, unused-or-broken seams rather than leaving them as dead code, since each mapped to a real requirement rather than speculative scope: `ClusterSession`/`WatchRegistry` (shared-watch, see 9.1), `save()`'s window-geometry capture (layout restore, see 9.1), and `PlaceholderPanel` in place of a real `LogsPanel` (log streaming, see 9.1) - `panel.rs` deleted once nothing referenced it. Kept as a deliberate single-instance generic: `ResourceIndex<Pod>` in `pods.rs` (a real generic container, just only one resource kind exists yet)
